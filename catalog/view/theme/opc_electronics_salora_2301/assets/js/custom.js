...
$(document).ready(function() {
	...
	function removeDefaultDropdown()
	{
		...
		$('#header .ttv-currency-dropdown').removeClass('open');
	...
	function showView(){
		if (document.body.clientWidth <= mobileViewSize) {//for mobile view
			 ...
			 moveDataInMobileView('#_desktop_search', '#ttvmobile_search');
			...
			moveDataInDesktopView('#_desktop_search', '#ttvmobile_search');
			// console.log('moveDataInDesktopView');
		}

		if (document.body.clientWidth <= 767) {//In mobile view
	...
	function DesktopMenu(ttv){
		if (document.body.clientWidth > mobileViewSize) 
		{
			...
			var menuTopOffset = $(".ttvcms-menu-wrapper").offset();//menu top margin or other spacing
			...
			if(popupFinalWidth > menuWidth){
				...
				var get_menu_length = 0;
				$('#ttv-top-menu > li').each(function(e){
					get_menu_length = $(this).width() + get_menu_length + 50; 
				});
				if (popupFinalWidth > get_menu_length) {
					popupFinalWidth = get_menu_length;
				}
			}
			if(popupFinalWidth > new_outer_width_popup){
				new_outer_width_popup = popupFinalWidth - 100;
			}
			...
			//$(obj).find('.ttvmain-menu-sub-menu').css('left', xLeft);
                        ...
                        pos.right = wWraper - (pos.left + $(obj).width());
                        posWraper.right = wWraper - (posWraper.left + wraper.width());
                        var xRight = pos.right - posWraper.right;
			if ((xRight + new_outer_width_popup) > wWraper) xRight = wWraper - new_outer_width_popup;
			$(obj).find('.ttvmain-menu-sub-menu').css('right', xRight);
	...
	$(document).on('click', '#ttvcmsmobile-horizontal-menu .ttv-has-child .ttvmenu-link-wrapper .ttvmenu-toggle-icon i', function() {
		if ($(this).hasClass('open')) {
			...
			$(this).addClass('open');
			$(this).parent().parent().next('.ttvmain-menu-mobile-dropdown').addClass('open').stop(false).slideDown(500);
	...
	$(document).on('click','.ttvcms-header-menu .ttvmain-menu-close', function(){
  	...
  	/********************* search title js ***************************/
	/*$(document).on('click','#header .ttvcmsheader-search .ttvsearch-open', function(){
		removeDefaultDropdown();
  		if(document.body.clientWidth >= 768 ) {
			$(this).addClass('open');
			$('.ttvcmssearch-wrapper').find('.ttvsearch-header-display-wrappper').addClass('open');
	        $('#header .ttvcmsheader-search .ttvsearch-close').show();
	...
	$(document).on('click','.ttvcms-header-menu .ttvmain-menu-open', function(){
  		if(document.body.clientWidth >= 768 ) {
			// $(this).addClass('open');
			$('.ttvcms-header-logo').addClass('open');
			$('.ttvcms-header-menu').find('.ttvmain-menu-icon').addClass('open');
	...
	$(document).on('click','.ttvcms-header-menu .ttvmain-menu-close', function(){
  		if(document.body.clientWidth >= 768 ) {
			/*$(this).removeClass('open');*/
			$('.ttvcms-header-logo').removeClass('open');
			$('.ttvcms-header-menu').find('.ttvmain-menu-icon').removeClass('open');
			$('body').removeClass('ttvactive-search');
		}
  	});
	/********************* end hedaer js ***************************/

	$(document).on('click','#header .ttvcmsheader-search .ttvsearch-close', function(){
		removeDefaultDropdown();
  		if(document.body.clientWidth >= 768 ) {
			$(this).removeClass('open');
			$('.ttvcmssearch-wrapper').find('.ttvsearch-header-display-wrappper').removeClass('open');
