    	...
    	if($this->config->get('theme_default_directory') == "opc_electronics_salora_2301"){
			...
			if(!empty($data['status_main_form'])){
				if(!empty($data['status_main_title'])){
					$data['tvcmscustomerservices_main_title'] 		= $setting['tvcmscustomerservices_main'][$land_id]['tvcmscustomerservices_main_title'];
				}
				if(!empty($data['status_main_short_description'])){
					$data['tvcmscustomerservices_main_short_des'] 	= $setting['tvcmscustomerservices_main'][$land_id]['tvcmscustomerservices_main_short_des'];
				}
				if(!empty($data['status_main_description'])){
					$data['tvcmscustomerservices_main_des'] 		= $setting['tvcmscustomerservices_main'][$land_id]['tvcmscustomerservices_main_des'];
				}
				if(!empty($data['status_main_image'])){
					$data['tvcmscustomerservices_main_img'] 		= $this->model_tool_image->resize($setting['tvcmscustomerservices_main'][$land_id]['tvcmscustomerservices_main_img'], $this->config->get('tvcmscustomsetting_customerservice_mainimg_width'),$this->config->get('tvcmscustomsetting_customerservice_mainimg_height'));
				}
			}
