...
class ControllerMarketingCoupon extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['coupons'][] = array(
				...
				'date_start' => ($this->language->get('code') == 'fa') ? jdate($this->config->get('config_khorshididate_format'), strtotime($result['date_start'])) : date($this->language->get('date_format_short'), strtotime($result['date_start'])),
				'date_end'   => ($this->language->get('code') == 'fa') ? jdate($this->config->get('config_khorshididate_format'), strtotime($result['date_end'])) : date($this->language->get('date_format_short'), strtotime($result['date_end'])),
	...
	protected function getForm() {
		...
		if (isset($this->request->post['date_start'])) {
			...
			$data['date_start'] = ($coupon_info['date_start'] != '0000-00-00' ? (($this->language->get('code') == 'fa') ? gtj($coupon_info['date_start']) : $coupon_info['date_start']) : '');
		} else {
			$data['date_start'] = ($this->language->get('code') == 'fa') ? jdate('Y-m-d', time()) : date('Y-m-d', time());
		}

		if (isset($this->request->post['date_end'])) {
			$data['date_end'] = $this->request->post['date_end'];
		} elseif (!empty($coupon_info)) {
			$data['date_end'] = ($coupon_info['date_end'] != '0000-00-00' ? (($this->language->get('code') == 'fa') ? gtj($coupon_info['date_end']) : $coupon_info['date_end']) : '');
		} else {
			$data['date_end'] = ($this->language->get('code') == 'fa') ? jdate('Y-m-d', strtotime('+1 month')) : date('Y-m-d', strtotime('+1 month'));
		}
	...
	public function history() {
		...
		foreach ($results as $result) {
			$data['histories'][] = array(
				...
				'date_added' => ($this->language->get('code') == 'fa') ? jdate($this->config->get('config_khorshididate_format'), strtotime($result['date_added'])) : date($this->language->get('date_format_short'), strtotime($result['date_added']))
			);
		}
