...
class ControllerExtensionReportSaleOrder extends Controller {
	...
	public function report() {
		...
		if (isset($this->request->get['filter_date_start'])) {
			...
			$filter_date_start = ($this->language->get('code') == 'fa') ? jdate('Y') . '-' . jdate('m') . '-01' : date('Y-m-d', strtotime(date('Y') . '-' . date('m') . '-01'));
		}

		if (isset($this->request->get['filter_date_end'])) {
			$filter_date_end = $this->request->get['filter_date_end'];
		} else {
			$filter_date_end = ($this->language->get('code') == 'fa') ? jdate('Y-m-d') : date('Y-m-d');
		}
