		...
		switch ($range) {
			...
			case 'week':
				...
				for ($i = 0; $i < 7; $i++) {
					...
					$json['xaxis'][] = ($this->language->get('code') == 'fa') ? array(date('w', strtotime($date)), jdate('l', strtotime($date))) : array(date('w', strtotime($date)), date('D', strtotime($date)));
				}
				break;
			case 'month':
			...
			case 'year':
				...
				for ($i = 1; $i <= 12; $i++) {
					$json['xaxis'][] = ($this->language->get('code') == 'fa') ? array($i, jdate('F', jmktime(0, 0, 0, $i, 1, jdate('Y')))) : array($i, date('M', mktime(0, 0, 0, $i)));
				}
				break;
		}
