...
class ControllerExtensionDashboardCustomer extends Controller {
	...
	public function dashboard() {
		...
		$today = $this->model_customer_customer->getTotalCustomers(array('filter_date_added' => ($this->language->get('code') == 'fa') ? jdate('Y-m-d', strtotime('-1 day')) : date('Y-m-d', strtotime('-1 day'))));

		$yesterday = $this->model_customer_customer->getTotalCustomers(array('filter_date_added' => ($this->language->get('code') == 'fa') ? jdate('Y-m-d', strtotime('-2 day')) : date('Y-m-d', strtotime('-2 day'))));
