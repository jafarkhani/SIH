...
class ControllerExtensionDashboardOrder extends Controller {
	...
	public function dashboard() {
		...
		$today = $this->model_sale_order->getTotalOrders(array('filter_date_added' => ($this->language->get('code') == 'fa') ? jdate('Y-m-d', strtotime('-1 day')) : date('Y-m-d', strtotime('-1 day'))));

		$yesterday = $this->model_sale_order->getTotalOrders(array('filter_date_added' => ($this->language->get('code') == 'fa') ? jdate('Y-m-d', strtotime('-2 day')) : date('Y-m-d', strtotime('-2 day'))));
