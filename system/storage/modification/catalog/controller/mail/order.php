...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		$data['date_added'] = $language->get('code') == 'fa' ? jdate($this->config->get('config_khorshididate_format'), strtotime($order_info['date_added'])) : date($language->get('date_format_short'), strtotime($order_info['date_added']));
