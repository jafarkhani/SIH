...
function jtg($jdate) {
	...
	if (isset($jdate_array[1])) {
		
		// Date Has Time
		$ymd = jtgcalc($jdate_array[0]);
		return $ymd . ' ' . $jdate_array[1];
	} else {
		return jtgcalc($jdate_array[0]);
	}
}

function jtgcalc($date) {
    if ($date!=0000-00-00)
	{
...
function gtj($date){
	...
	{
	$date = tr_num($date);	
	list( $gyear, $gmonth, $gday ) = preg_split ( '/-/', $date); 
...
function jdate($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='en'){
 ...
 for($i=0; $i<$sl; $i++){
  ...
  switch($sub){
	...
	case'A':
	$out.=($date[0]<12)?'پیش از ظهر':'بعد از ظهر';
	break;
...
function jstrftime($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
 ...
 for($i=0; $i<$sl; $i++){
  ...
  switch($sub){
	...
	case'p':
	$out.=($date[1]<12)?'پیش از ظهر':'بعد از ظهر';
	break;
	...
	case'r':
	$out.=$date[0].':'.$date[2].':'.$date[5].' '.(($date[1]<12)?'پیش از ظهر':'بعد از ظهر');
	break;
...
/** Convertor from and to Gregorian and Jalali (Hijri_khorshidi,Solar) Functions
Copyright(C)2011, Reza Gholampanahi [ http://jdf.scr.ir/jdf ] version 2.50 */

/*	F	*/
function gregorian_to_jalali($g_y,$g_m,$g_d,$mod=''){
	$g_y=tr_num($g_y); $g_m=tr_num($g_m); $g_d=tr_num($g_d);/* <= :این سطر ، جزء تابع اصلی نیست */
 $d_4=$g_y%4;
...
function jalali_to_gregorian($j_y,$j_m,$j_d,$mod=''){
	$g_y=tr_num($j_y); $j_m=tr_num($j_m); $j_d=tr_num($j_d);/* <= :این سطر ، جزء تابع اصلی نیست */
 $d_4=($j_y+1)%4;
